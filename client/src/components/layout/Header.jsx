...
//         ].join(' ')}
//       >
//         {/* ⬇️ CHANGE #1: max-w-7xl aur mx-auto add kiya, px-4 kiya ⬇️ */}
//         <div className="w-full px-4 flex items-center h-24 max-w-7xl mx-auto">
//           {/* ── Logo ────────────────────────────────────── */}
//           {/* ⬇️ CHANGE #2: -ml-4 aur pl-1 add kiya ⬇️ */}
//           <Link to="/" className="flex items-center flex-shrink-0 -ml-12 pl-1">
//             <img
//               src={logo}
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className={[
          'flex items-center gap-1 px-4 py-2 text-sm font-medium rounded-md transition-all duration-150',
          isActive
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-56 bg-white rounded-lg border border-gray-200 shadow-xl z-50 py-1 animate-fade-in-down">
          {item.children.map((child) => (
            <Link
              key={child.path}
              to={child.path}
              ...
              className="block px-4 py-2.5 text-sm text-gray-600 hover:text-red-700 hover:bg-red-50 transition-colors duration-150"
            >
              {child.label}
            </Link>
          ))}
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="hidden md:block bg-gray-900">
        <div className="max-w-7xl mx-auto px-4 h-10 flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-4 text-gray-300">
            ...
            <span className="hidden xl:block text-gray-600">|</span>
            <a
              href="https://maps.google.com/?q=8193+Elder+Creek+Road,+Sacramento,+CA+95824"
              target="_blank"
              rel="noopener noreferrer"
      ...
      >
        <div className="w-full px-4 flex items-center h-20 max-w-7xl mx-auto">
          {/* Logo */}
          <Link to="/" className="flex items-center flex-shrink-0 -ml-8 pl-1">
            <img
              src={logo}
              alt="Carnex Auto Sales"
              className="h-[70px] w-auto object-contain transition-transform duration-300 hover:scale-105 drop-shadow-md"
            />
          </Link>
          ...
          <div className="flex md:hidden items-center gap-2 ml-auto">
            <Link
              to="/inventory"
              className="px-3 py-1.5 text-xs font-semibold text-white bg-red-700 rounded-md hover:bg-red-800 transition-colors flex items-center gap-1"
            >
              <PlusIcon />
              <span>Inventory</span>
            </Link>
            <button
              ...
              aria-label={mobileOpen ? 'Close menu' : 'Open menu'}
              aria-expanded={mobileOpen}
              className="w-9 h-9 flex items-center justify-center rounded-md border border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-800 transition-colors duration-150"
            >
              {mobileOpen ? <CloseIcon /> : <MenuIcon />}
            </button>
          </div>
          ...
          <div className="hidden md:flex lg:hidden items-center ml-1">
            <Link
              to="/inventory"
              className="px-3 py-1.5 text-xs font-semibold text-white bg-red-700 rounded-md hover:bg-red-800 transition-colors flex items-center gap-1 flex-shrink-0"
            >
              <PlusIcon />
              <span>Inventory</span>
            </Link>
      ...
      {mobileOpen && (
        <>
          ...
          <div
            ref={mobileMenuRef}
            className="absolute top-full left-0 right-0 bg-white border-t border-gray-200 shadow-2xl z-50 lg:hidden animate-fade-in-down max-h-[calc(100vh-120px)] overflow-y-auto"
          >
            {/* Mobile contact bar */}
            <div className="bg-gray-900 px-4 py-3 flex flex-col gap-2 text-xs text-gray-300">
              <a
                href={`tel:${DEALERSHIP.phone}`}
                className="flex items-center gap-1.5 hover:text-white transition-colors"
              >
              ...
              {MAIN_NAV.map((item) => (
                <div key={item.path}>
                  <NavLink
                    ...
                    onClick={() => setMobileOpen(false)}
                    className={({ isActive }) =>
                      `flex items-center justify-between px-4 py-3 text-sm font-medium rounded-md transition-colors duration-150 ${
                        isActive
                    ...
                    <div className="ml-4 mt-0.5 space-y-0.5 pb-1 border-l-2 border-red-200">
                      {item.children.map((child) => (
                        <NavLink
                          ...
                          to={child.path}
                          onClick={() => setMobileOpen(false)}
                          className="block px-4 py-2.5 text-sm text-gray-600 hover:text-red-700 hover:bg-red-50 rounded-md transition-colors duration-150"
                        >
                          {child.label}
                        </NavLink>
                      ))}
            ...
            <div className="px-4 pb-5 pt-2 border-t border-gray-200 space-y-2.5">
              <Link
                to="/inventory"
                onClick={() => setMobileOpen(false)}
                className="w-full py-3 text-sm font-semibold text-white bg-red-700 rounded-md hover:bg-red-800 transition-colors flex items-center justify-center gap-2"
              >
                <PlusIcon />
                Browse All Inventory
              </Link>
              <Link
                to="/contact"
                onClick={() => setMobileOpen(false)}
                className="w-full py-3 text-sm font-medium text-gray-700 border border-gray-300 rounded-md hover:border-red-700 hover:text-red-700 transition-all duration-150 flex items-center justify-center"
              >
                Contact Dealership
              </Link>
              <a
                href={`tel:${DEALERSHIP.phone}`}
                className="w-full py-3 text-sm font-medium text-gray-600 bg-gray-50 rounded-md hover:bg-gray-100 hover:text-red-700 transition-colors flex items-center justify-center gap-2"
              >
                <PhoneIcon />
                {DEALERSHIP.phone}
              </a>
